...
export default function LessonManagerModal({ user, onClose }) {
  ...
  function close() { if (!processing.current) onClose(); }
  function keyboard(event) {
    if (event.key === "Escape" && !deleting) { event.stopPropagation(); close(); }
    if (event.key !== "Tab" || deleting) return;
    const controls = [...modal.current.querySelectorAll('button:not(:disabled), input:not([hidden]):not(:disabled), a[href], summary')]
  ...
  return <div className="modal-backdrop" {...backdropClose(close)} onKeyDown={keyboard}>
    <section ref={modal} className="modal modal-lesson lesson-file-manager" role="dialog" aria-modal="true" aria-labelledby="lesson-files-title">
      <div className="modal-head">
        ...
        <button type="button" className="btn-close" onClick={close} disabled={!!busy} aria-label="닫기">×</button>
      </div>
      <div className="lesson-file-toolbar">
        ...
        <button type="button" className="btn-primary" disabled={!!busy || loading || !me?.uid} onClick={() => input.current?.click()}>
          <IconAddFeature size={18} /> 파일 올리기
        </button>
        <input ref={input} type="file" aria-label="자료 파일" accept={LESSON_FILE_ACCEPT} multiple hidden onChange={upload} disabled={!!busy} />
      </div>
      ...
      <div className="lesson-list">
        {loading ? <p className="empty-note">자료를 불러오는 중입니다.</p> : visible.length === 0 ? <p className="empty-note">{query ? "검색 결과가 없습니다." : "저장된 파일이 없습니다."}</p> :
          visible.map(file => <div className="lesson-row lesson-file-row" key={file.id}>
            <div className="lesson-row-main"><strong>{file.name}</strong><span>{file.extension.toUpperCase()} · {file.size < 1024 * 1024 ? Math.max(1, Math.ceil(file.size / 1024)) + " KB" : (file.size / 1024 / 1024).toFixed(1) + " MB"}</span></div>
            <div className="lesson-row-actions">
              <button type="button" className="btn-outline" disabled={!!busy} onClick={() => fileAction(file, false)}>다운로드</button>
              <button type="button" className="btn-ghost" title="배포 링크 복사" aria-label={file.name + " 배포 링크 복사"} disabled={!!busy} onClick={() => fileAction(file, true)}><IconDuplicate size={18} /></button>
              <button type="button" className="btn-ghost qa-delete" title="삭제" aria-label={file.name + " 삭제"} disabled={!!busy} onClick={() => setDeleting(file)}><IconTrash size={18} /></button>
            </div>
          </div>)}
      </div>
    ...
    {deleting && <ConfirmModal title="자료 삭제" preview={deleting.name} description="원본 파일과 배포 링크가 삭제됩니다. 되돌릴 수 없습니다." confirmLabel="삭제" danger confirmDisabled={!!busy} cancelDisabled={!!busy} onConfirm={remove} onClose={() => { if (!processing.current) setDeleting(null); }} />}
  </div>;
}
