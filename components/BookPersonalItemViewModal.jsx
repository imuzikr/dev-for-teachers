        ...
        <div className="book-personal-expand-body">
          ...
          {requiresAnswer && !hideResponse && (
            <section className="book-personal-expand-response" aria-label={interactive ? "나의 답변" : "학생 답변"}>
              ...
              {interactive && onAnswerChange ? <textarea aria-label="답변 내용" value={answerDraft} onChange={(event) => onAnswerChange(event.target.value)} disabled={saving} /> : <p>{response || "아직 입력한 내용이 없습니다."}</p>}
            </section>
          )}
          {interactive && (
