...
import { deleteBookProject } from "@/lib/store";
import { deleteBookProjectConfirmations } from "@/lib/bookConfirmations";
import ConfirmModal from "./ConfirmModal";
import { IconTrash } from "./StatusIcons";

export default function BookProjectDeleteButton({ user, project, disabled, onDeleted, onPendingChange, removeProject = deleteBookProject }) {
  ...
  async function confirm() {
    ...
    setError("");
    try {
      await removeProject(user, { classId: target.classId || target.id, version: target.version }, deleteBookProjectConfirmations);
      setTarget(null);
      onDeleted?.();
    } catch (failure) {
      console.error("[책방] 프로젝트 삭제 실패:", failure);
      setError(failure?.code === "book-project/stale" ? failure.message : "삭제를 완료하지 못했어요. 일부 기록은 이미 삭제되었을 수 있습니다. 다시 시도해 주세요.");
    } finally {
      busy.current = false;
      setPending(false);
  ...
  return <>
    ...
    {target && createPortal(<div ref={modalRef}><ConfirmModal
      ...
      description={<>이 프로젝트의 모든 Step, 활동, 자료와 학생 작성·확인 기록이 삭제됩니다. 반과 참여자 명단은 유지됩니다. 되돌릴 수 없습니다.
