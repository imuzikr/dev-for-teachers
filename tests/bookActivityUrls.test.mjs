...
test("Firestore submission paths are per student and optional fields preserve existing entries", async () => {
  ...
  assert.deepEqual(plain(combined.data.answers), { K: "keep answer" });
  ...
  assert.equal(api.writes.at(-1).data.dashboardText, "result");
  ...
  assert.deepEqual(plain(api.writes.at(-1).data.urls), ["https://example.com/new"]);
