...
async function collectNestedDeleteRefs(db, classId) {
  ...
  refs.push(...(await docsIn(collection(db, "broadcasts", classId, "portfolioChunks"))).map((snapshot) => snapshot.ref));
...
async function collectTopDeleteRefs(db, classId) {
  const refs = [];
  for (const name of ["bookResources", "bookHelpNotes", "bookProjects", "bookProjectTrash", "bookConfirmations", "broadcasts", "memberships", "presence", "studentNotes", "classJoinClaims", "classJoinLookup", "rewards", "kwl"]) {
...
async function collectSurvivingRefs(db, deletePaths) {
  const snapshots = [];
  for (const name of ["classes", "lessons", "questions", "notices", "users", "bookResources", "bookProjects", "bookProjectTrash", "bookHelpNotes", "bookConfirmations", "broadcasts", "memberships", "presence", "studentNotes", "classJoinClaims", "classJoinLookup", "rewards", "kwl"]) {
...
async function unlinkLessonPublications(db, classId) {
  const publications = await docsIn(collection(db, "classes", classId, "lessonFiles"));
  for (const publication of publications) {
    const ownerId = publication.data().ownerId;
    if (typeof ownerId !== "string" || !ownerId || ownerId.includes("/")) {
      throwClient("배포 자료의 소유자 정보를 확인할 수 없어 반 삭제를 중단했어요.", "class-deletion/lesson-unlink-failed");
    }
    const batch = writeBatch(db);
    batch.update(doc(db, "lessonFiles", ownerId, "files", publication.id),
      new FieldPath("sharedClasses", classId), deleteField(), "lastSharedClassId", classId);
    batch.delete(publication.ref);
    try {
      await batch.commit();
    } catch (error) {
      const failure = new ClassDeletionClientError("자료 배포 해제에 실패해 반 삭제를 중단했어요. 원본 자료는 유지됩니다. 잠시 후 다시 시도해 주세요.", "class-deletion/lesson-unlink-failed");
      failure.cause = error;
      throw failure;
    }
  }
...
export async function deleteClassInBrowser(classId, options = {}) {
  ...
  const deletedPublications = await unlinkLessonPublications(db, classId);
  ...
  return { status: "completed", deletedDocuments: deletedPublications + deletedChildren + deletedRoot, deletedFiles, retainedFiles };
}
