...
// 허용 태그: 볼드, 이탤릭, 밑줄, 목록, 줄바꿈, 코드 블록, 제한된 크기 표시, 체크리스트
...
const RICH_TEXT_TAG_PATTERN = /<\/?(b|strong|i|em|u|span|ul|ol|li|br|div|p|pre|code|img|input|label)\b/i;
...
function allowedClassesForElement(el) {
  return [...el.classList].filter((className) => {
    if (!ALLOWED_CLASSES.has(className)) return false;
    if (className === "rte-checklist") return el.tagName === "UL";
    if (className === "rte-checklist-text") return el.tagName === "SPAN";
    return true;
  });
}

function isChecklistCheckbox(el) {
  ...
  return (
    ...
    item?.tagName === "LI" &&
    list?.tagName === "UL" &&
    list.classList.contains("rte-checklist")
  );
}
  ...
  function walk(node) {
    ...
    [...node.children].forEach((el) => {
      ...
      if (!ALLOWED_TAGS.has(el.tagName)) {
        ...
        const isAllowedChecklistCheckbox = isChecklistCheckbox(el);
        const checked = el.hasAttribute("checked");
        [...el.attributes].forEach((a) => el.removeAttribute(a.name));
        if (isAllowedChecklistCheckbox) {
        ...
        const classes = allowedClassesForElement(el);
        // 허용 태그여도 속성(onclick, style 등)은 모두 제거하되,
        // 서식 에디터가 만든 제한된 크기 class만 보존합니다.
        [...el.attributes].forEach((a) => el.removeAttribute(a.name));
...
export function safeDisplayHtml(html = "") {
