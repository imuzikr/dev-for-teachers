...
export function sanitizeHelpSection(section) {
  return {
    id: assertStringLimit(section?.id, "section.id", 100, { required: true }),
    title: assertStringLimit(section?.title, "section.title", 120, { required: true }),
    content: assertStringLimit(section?.content, "section.content", 5000),
    url: assertStringLimit(section?.url, "section.url", 1000),
  };
}

function sanitizeHelpSections(sections) {
...
function normalizeHelpSectionsForRead(sections) {
  ...
  return sections.slice(0, MAX_HELP_SECTIONS).flatMap((section) => {
    try {
...
function normalizeHelpNote(id, data) {
  return {
    ...
    sections: normalizeHelpSectionsForRead(data?.sections),
  };
}
...
function upsertHelpSection(sections, section, create) {
  ...
  return sections.map((item, index) => (index === existingIndex ? normalized : item));
}
...
export async function saveBookHelpSection(noteId, section, { create = false } = {}) {
  if (isFirebaseConfigured) {
    ...
    await runTransaction(db, async (transaction) => {
      ...
      const sections = sanitizeHelpSections(data?.sections);
  ...
  target.sections = upsertHelpSection(sanitizeHelpSections(target.sections), section, create);
...
export async function deleteBookHelpSection(noteId, sectionId) {
  if (isFirebaseConfigured) {
    ...
    await runTransaction(db, async (transaction) => {
      ...
      const data = snapshot.data();
      transaction.update(noteRef, {
        sections: removeHelpSection(sanitizeHelpSections(data?.sections), sectionId),
        updatedAt: serverTimestamp(),
      });
    });
    return;
  ...
  const target = findMockHelpNote(noteId);
  target.sections = removeHelpSection(sanitizeHelpSections(target.sections), sectionId);
  target.updatedAt = new Date();
  mockHelpListeners.forEach((listener) => listener());
}
