...
export function saveGuestTeacherSession({ uid, schoolName, teacherName }) {
  ...
  sessionStorage.setItem(GUEST_TEACHER_KEY, serialized);
  localStorage.setItem(GUEST_TEACHER_KEY, serialized);
  window.dispatchEvent(new Event("role-change"));
  return profile;
}
...
export function getGuestTeacherSession() {
  if (typeof window === "undefined") return null;
  try {
    const raw = localStorage.getItem(GUEST_TEACHER_KEY)
      ?? sessionStorage.getItem(GUEST_TEACHER_KEY);
    const saved = JSON.parse(raw ?? "null");
    if (saved?.uid && saved?.schoolName && saved?.teacherName) {
...
// 최고 관리자 — Firestore가 보호하는 프로필의 admin 역할만 신뢰합니다.
