...
export async function requestPinAuth(payload) {
  ...
  try {
    return await sendPinAuth(payload, token);
  } catch (error) {
    if (!["begin", "register"].includes(payload.action)
      || error?.code !== "auth/pin-enrollment-required") throw error;
    try {
      await auth.authStateReady();
      token = await auth.currentUser?.getIdToken();
    } catch {
    ...
    return sendPinAuth(payload, token);
  }
}

async function sendPinAuth(payload, token) {
