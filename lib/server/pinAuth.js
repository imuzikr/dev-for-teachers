...
async function requirePinLoginTarget(services, uid) {
  const profile = await requireExistingProfile(services.db, uid);
  if (!isStudentProfile(profile) || await isSystemAdminUid(services.db, uid)) {
    throw new PinAuthHttpError(403, "auth/pin-target-forbidden", "Only student accounts can use PIN login.");
  }
  const userRecord = await safeGetUser(services.auth, uid);
