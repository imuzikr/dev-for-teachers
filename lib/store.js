...
import { CLASS_PURPOSE_INTERNAL, getClassPurpose, normalizeClassPurpose } from "./classPurpose";
...
export function classAcceptsJoin(classItem) {
  return !!classItem && !classItem.archived && classItem.joinEnabled === true && isValidClassJoinCode(classItem.joinCode);
}
...
async function joinInheritedClass(classId, user, inheritedFromClassId) {
  if (isFirebaseConfigured) {
...
async function joinInternalSiblingClasses(target, user) {
  if (getClassPurpose(target) !== CLASS_PURPOSE_INTERNAL || !target.createdBy) return;

  if (isFirebaseConfigured) {
    const snap = await getDocs(query(collection(db, "classes"), where("createdBy", "==", target.createdBy)));
    const siblings = snap.docs
      .map((d) => ({ id: d.id, ...d.data() }))
      .filter((classItem) => (
        classItem.id !== target.id
        && !classItem.archived
        && getClassPurpose(classItem) === CLASS_PURPOSE_INTERNAL
      ));
    await Promise.all(siblings.map((classItem) => joinInheritedClass(classItem.id, user, target.id)));
    return;
  }
  ...
  mock.classes
    .filter((classItem) => (
      classItem.id !== target.id
      && classItem.createdBy === target.createdBy
      && !classItem.archived
      && getClassPurpose(classItem) === CLASS_PURPOSE_INTERNAL
    ))
    .forEach((classItem) => {
      if (mock.memberships.some((membership) => membership.uid === user.uid && membership.classId === classItem.id)) return;
      mock.memberships.push({
        id: `${user.uid}_${classItem.id}`,
        uid: user.uid,
        classId: classItem.id,
        joinCode: "",
        inheritedFromClassId: target.id,
        joinedAt: new Date(),
      });
    });
...
export async function addClass(user, name, options = {}) {
  ...
  if (isFirebaseConfigured) {
    ...
    await copyClassMembershipsToClass(
      purpose === CLASS_PURPOSE_INTERNAL ? options.copyFromClassIds : [],
      ref.id
    );
    return { id: ref.id, name: name.trim(), purpose, joinEnabled: true, joinCode };
  }
...
export async function updateClassJoinAccess(classId, { joinEnabled, joinCode }) {
  const patch = {};
  if (typeof joinEnabled === "boolean") patch.joinEnabled = joinEnabled;
  if (joinCode !== undefined) patch.joinCode = normalizeClassJoinCode(joinCode);
...
export async function joinClass(classId, user, joinCode = "") {
...
export async function joinClassByCode(code, user) {
  const joinCode = normalizeClassJoinCode(code);
  if (!joinCode) {
    throw Object.assign(new Error("참여 코드를 입력해 주세요."), { code: "missing-code" });
  }
  ...
  if (isFirebaseConfigured) {
    const snap = await getDocs(query(collection(db, "classes"), where("joinEnabled", "==", true)));
    const matches = snap.docs
      .map((d) => ({ id: d.id, ...d.data() }))
      .filter((classItem) => (
        normalizeClassJoinCode(classItem.joinCode) === joinCode && classAcceptsJoin(classItem)
      ));
    if (matches.length === 0) {
      throw Object.assign(new Error("참여 가능한 반을 찾지 못했습니다."), { code: "class-not-found" });
    }
    const target = matches[0];
    await joinClass(target.id, user, joinCode);
    await joinInternalSiblingClasses(target, user).catch((error) =>
      console.warn("[class-membership] 교내용 차시 연결 일부 실패:", error)
    );
    return target;
  }

  const target = mock.classes.find((classItem) => (
    normalizeClassJoinCode(classItem.joinCode) === joinCode && classAcceptsJoin(classItem)
  ));
  if (!target) {
    throw Object.assign(new Error("참여 가능한 반을 찾지 못했습니다."), { code: "class-not-found" });
  ...
  await joinClass(target.id, user, joinCode);
  await joinInternalSiblingClasses(target, user);
  return target;
}
