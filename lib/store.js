...
export async function saveBookDashboardText(actId, user, dashboardText, urls, images) {
  const data = {
    activityId: actId,
    authorId: user.uid,
    authorName: user.realName || user.displayName || "이름 미설정",
  ...
  if (isFirebaseConfigured) {
    ...
    await runTransaction(db, async (transaction) => {
      ...
      assertEntrySize({ ...(current.exists() ? current.data() : {}), ...data });
      transaction.set(entryRef, { ...data, updatedAt: serverTimestamp() }, { merge: true });
    });
    return;
  }
  ensureMockBooks();
