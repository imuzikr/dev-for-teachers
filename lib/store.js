...
export const BOOK_ACTIVITY_TYPES = ["mindmap"];
export const BOOK_SOLO_TYPES = ["mindmap"];
...
export async function saveBookProject(user, { classId, title, steps }) {
  ...
  if (isFirebaseConfigured) {
    ...
    const storedSteps = preparedSteps.map((step, stepIndex) => {
      ...
      const activities = step.activities.map((activity, activityIndex) => {
        ...
        return { ...activity, id: activityRef.id, type: "mindmap" };
      });
  ...
  const storedSteps = preparedSteps.map((step, stepIndex) => {
    ...
    const activities = step.activities.map((activity, activityIndex) => {
      const existingActivity = existingActivityIds.has(activity.id);
      const id = existingActivity ? activity.id : `ba${++mockSeq}_m`;
      ...
      if (existingIndex >= 0) mock.bookActivities[existingIndex] = { ...mock.bookActivities[existingIndex], ...storedActivity };
      else mock.bookActivities.unshift(storedActivity);
      return { ...activity, id, type: "mindmap" };
    });
    const resources = step.resources.map((resource, resourceIndex) => ({
      ...resource,
      resourceId: resource.id,
...
export async function addBookActivity(
  ...
  const kind = BOOK_ACTIVITY_TYPES.includes(type) ? type : "mindmap";
  const defaultTitle = "마인드맵";
  const data = {
    classId,
    type: kind,
