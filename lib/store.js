...
export async function deleteClass(classId) {
  if (isFirebaseConfigured) {
    ...
    await Promise.all(activitiesSnap.docs.map((activity) => deleteBookActivity(activity.id)));
    const resourcesSnap = await getDocs(
      query(collection(db, "bookResources"), where("classId", "==", classId))
    );
    await Promise.all(resourcesSnap.docs.map((resource) => deleteDoc(resource.ref)));
    const confirmationsSnap = await getDocs(
      query(collection(db, "bookConfirmations"), where("classId", "==", classId))
    );
    await Promise.all(confirmationsSnap.docs.map((confirmation) => deleteDoc(confirmation.ref)));
    await deleteDoc(doc(db, "bookProjects", classId));
    ...
    await runTransaction(db, async (transaction) => {
      ...
      const classSnap = await transaction.get(classRef);
      const secretSnap = await transaction.get(secretRef);
      const codes = [...new Set([classSnap.data()?.joinCode, secretSnap.data()?.joinCode]
        .filter((code) => typeof code === "string" && /^[0-9]{6}$/.test(code)))];
      const lookups = [];
      for (const code of codes) {
        const ref = doc(db, "classJoinLookup", code);
        lookups.push({ ref, snap: await transaction.get(ref) });
      }
      for (const { ref, snap } of lookups) {
        if (snap.exists() && snap.data().classId === classId) transaction.delete(ref);
      }
...
export async function saveBookProject(user, { classId, title, steps }) {
  ...
  if (isFirebaseConfigured) {
    ...
    batch.set(doc(db, "bookProjects", projectId), projectData);
    await batch.commit();
    return;
  }
