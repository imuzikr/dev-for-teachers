...
import { assertBookProjectSize, normalizeBookItemImages } from "./bookProjectImages";
import {
  collection,
  addDoc,
  setDoc,
    ...
    const storedSteps = preparedSteps.map((step, stepIndex) => {
      ...
      const resources = step.resources.map((resource, resourceIndex) => {
        batch.set(doc(db, "bookResources", resource.id), {
          ...
          ...(resource.images.length > 0 ? { images: resource.images } : {}),
          url: resource.url,
...
export async function updateBookActivity(actId, patch) {
  if ("images" in patch) patch = { ...patch, images: normalizeBookItemImages(patch.images) };
  if (isFirebaseConfigured) {
