...
export async function updateBookBroadcastScroll(classId, sessionId, scrollPosition) {
  ...
  if (isFirebaseConfigured) {
    ...
    await runTransaction(db, async (transaction) => {
      ...
      if (!snap.exists() || snap.data().mode !== "bookItem" || snap.data().scrollSessionId !== sessionId) return;
  ...
  if (current?.mode !== "bookItem" || current.scrollSessionId !== sessionId) return;
...
export async function deleteBookProject(user, { classId, version }, cleanupConfirmations = async (...args) => {
  ...
  if (isFirebaseConfigured) {
    ...
    await runTransaction(db, async (transaction) => {
      ...
      if (broadcast.exists() && broadcast.data().mode === "bookItem" && broadcast.data().projectId === classId) {
  ...
  if (mock.broadcasts?.[classId]?.mode === "bookItem" && mock.broadcasts[classId].projectId === classId) {
