...
export async function saveBookProject(user, { classId, title, steps }) {
  ...
  const existingResourceIds = new Set(
    (existingProject?.steps ?? []).flatMap((step) => (step.resources ?? []).map((resource) => resource.id))
  );
  ...
  if (isFirebaseConfigured) {
    ...
    const storedSteps = preparedSteps.map((step, stepIndex) => {
      ...
      const activities = step.activities.map((activity, activityIndex) => {
        ...
        if (existingActivity) batch.set(activityRef, activityData, { merge: true });
        else batch.set(activityRef, activityData);
        return { ...activity, id: activityRef.id, type: "book" };
      });
      const resources = step.resources.map((resource, resourceIndex) => {
        batch.set(doc(db, "bookResources", resource.id), {
    ...
    const storedResourceIds = new Set(
      storedSteps.flatMap((step) => (step.resources ?? []).map((resource) => resource.id))
    );
    existingResourceIds.forEach((resourceId) => {
      if (!storedResourceIds.has(resourceId)) batch.delete(doc(db, "bookResources", resourceId));
    });
    const projectData = {
      classId,
      title,
    ...
    batch.set(doc(db, "bookProjects", projectId), projectData, { merge: true });
    await batch.commit();
    return;
  }
  ...
  mock.bookResources = (mock.bookResources ?? []).filter((resource) => (
    resource.classId !== classId || storedResourceIds.has(resource.id)
  ));
