...
function activeBookItemValue(kind, itemId) {
  if (!["activity", "resource"].includes(kind) || !itemId) {
...
export async function setBookActiveItem(classId, stepId, kind, itemId) {
  if (!classId || !stepId) {
    throw Object.assign(new Error("활성 항목을 저장할 프로젝트와 단계가 필요합니다."), { code: "book-project/missing-context" });
  }
  const activeValue = activeBookItemValue(kind, itemId);
  if (isFirebaseConfigured) {
    ...
    await runTransaction(db, async (transaction) => {
      ...
      transaction.update(projectRef, {
        activeItemByStep: { ...(project.activeItemByStep ?? {}), [stepId]: activeValue },
        updatedAt: serverTimestamp(),
      });
    });
    return;
  ...
  mock.bookProjects = mock.bookProjects.map((item) => item.classId === classId
    ? {
      ...item,
      activeItemByStep: { ...(item.activeItemByStep ?? {}), [stepId]: activeValue },
      updatedAt: new Date(),
    }
    : item);
  notifyBookProjects();
