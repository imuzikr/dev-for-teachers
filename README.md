...
- 사이드 패널의 **프로젝트 삭제**를 누르고 이름을 확인하면 프로젝트의 Step, 활동, 자료와 학생 작성·확인 기록을 삭제합니다. 반과 참여자 명단은 유지됩니다. 삭제 중 오류가 발생하면 확인창에서 다시 시도할 수 있습니다.
- **프로젝트 만들기 → 다른 반 프로젝트 복사**에서 접근 가능한 다른 반을 선택하고 **이 프로젝트 불러오기**를 누릅니다. 제목과 구성을 수정한 뒤 **프로젝트 저장**을 누르면 현재 반에 독립된 프로젝트가 만들어집니다. 학생 기록과 발표 중인 항목 상태는 복사하지 않습니다.
- 관련 회귀 테스트: `npm run test:projects`. 브라우저 검증용 격리 데모는 `node tests/prepareBookProjectManagementUi.mjs`로 생성합니다.
...
- **일반 선생님 입장**: 학교 이름과 이름을 입력하면 익명 세션을 만들고
  `/books`로 이동합니다. 민감 데이터 저장을 전제로 하지 않는 빠른 입장 흐름입니다.
...
app/books/page.js    책방 활동과 수업 운영
