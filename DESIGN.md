...
- `book-project-item-actions`: 저장된 활동·자료에는 관리자의 수정·삭제 명령을 제공하고, 자료에는 본문과 URL을 클립보드에 담는 복사 명령을 제공한다. 삭제는 확인 절차를 거친다.
- `book-personal-card`: 학교명, 이름, 활동 진행률을 표시한다. 교사는 참여자 전체를 조회하되 카드를 열지 않으며, 일반 사용자는 자신의 카드만 열어 교사가 준비한 활동 목록과 진행 상태를 카드 형태로 확인한다.
- `book-personal-modal`: 일반 사용자의 본인 카드에서만 열리는 넓은 상세 모달. 활동 제목·내용·진행 상태를 읽기 쉬운 카드 목록으로 제공하고 각 활동을 바로 시작할 수 있다.
