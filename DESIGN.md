...
- `book-library-side`: 참고 앱의 사이드바처럼 현재 작업 맥락을 압축해 보여 주는 책방 내비게이션 패널이다. 접기/펼치기 명령은 패널 오른쪽 상단의 작은 원형 `»` 아이콘 버튼으로 제공하고, 접힌 상태에서는 48px 어두운 레일 안에 같은 아이콘만 남긴다. 펼친 상태에서는 프로젝트 전체 Step·활동·자료·참여자 수 요약과 Step 아코디언 흐름을 제공하며, 별도의 빠른 이동 목록과 별도의 Step 상세 목록을 이원화하지 않는다.
