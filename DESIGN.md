...
- `book-library-side`: 참고 앱의 사이드바처럼 현재 작업 맥락을 압축해 보여 주는 책방 내비게이션 패널이다. 패널 상단에는 접기/펼치기 명령을 두고, 접힌 상태에서도 세로 라벨로 책방 흐름임을 알 수 있어야 한다. 펼친 상태에서는 프로젝트 전체 Step·활동·자료·참여자 수 요약과 Step 빠른 이동 목록을 제공한다.
...
- Step 빠른 이동 항목은 44px 이상의 터치 영역, 현재 Step 강조, 활동·자료 개수 배지를 제공한다. 긴 한글 제목은 말줄임 처리하되 버튼 내부 텍스트가 겹치거나 잘리지 않아야 한다.
