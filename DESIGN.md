...
- `book-project-step`: 활동과 자료를 구획으로 분리해 각각 복수로 담는 아코디언 카드. 저장된 Step 헤더에는 해당 Step을 바로 여는 편집 명령을 둔다. 편집 중인 Step 제목은 아코디언 헤더에서 바로 수정하고 같은 헤더에서 Step을 삭제할 수 있다. 본문 하단에는 활동 추가와 자료 추가 명령을 나란히 두고, 입력을 확정하는 Step 저장 명령을 제공한다. 각 활동·자료 박스는 제목을 수정하는 헤더와 별도 입력 영역을 사용하며, 활동은 활동 내용, 자료는 자료 내용과 선택 URL을 입력한다.
