...
- `book-personal-card`: 학교명, 이름, 활동 진행률을 표시한다. 교사는 참여자 전체를 조회하되 카드를 열지 않으며, 일반 사용자는 자신의 카드만 열어 교사가 준비한 활동 목록과 진행 상태를 카드 형태로 확인한다.
- `book-personal-detail`: 일반 사용자의 본인 카드에서만 진입하는 오른쪽 영역의 상세 대시보드. 교사가 준비한 활동을 순서 번호·제목·내용·진행 상태와 함께 카드로 나열하고 각 활동을 바로 시작하거나 다시 열 수 있다. 뒤로 명령으로 개인 카드 목록으로 돌아간다.
