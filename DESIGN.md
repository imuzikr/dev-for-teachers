...
- 학생 활동의 본문과 첨부 이미지 사이에 선택 입력인 `활동 URL` 목록을 둔다. 빈 목록은 입력 한 줄을 보여주고 마지막 줄 오른쪽 끝의 `+` 버튼으로 칸을 추가하고 새 칸에 포커스를 옮긴다. 추가·삭제 버튼은 44px 조작 영역과 기존 색상·입력·포커스 토큰을 사용한다. 학생은 체크리스트 및 확인 완료 여부와 관계없이 자신의 URL을 입력·수정·삭제하고 `URL 저장`으로 따로 저장한다. 일반 활동 저장에도 URL을 포함한다. 저장한 HTTP(S) 링크는 학생 패널·확대 보기와 교사의 해당 학생 상세 보기에서 표시하며, 긴 주소는 줄바꿈하고 새 탭으로 연다. 학생별 활동 기록에만 저장하며 기존 교사 제공 링크는 유지한다. 빈 입력은 저장에서 제외하고 오류 발생 시 입력을 보존한다.
