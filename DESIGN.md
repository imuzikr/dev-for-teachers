...
- 자료의 복사할 내용은 서식을 포함해 100,000자까지 저장한다. 초과하면 업로드 전에 자료 이름과 제한을 안내하고 입력을 보존한다. 프로젝트 저장 권한 오류는 일시적 재시도 오류와 구분해 반 접근 권한 및 Firestore 규칙 적용 여부를 안내한다. 기존 모달 오류 영역과 저장 버튼을 재사용한다.

- 교사 프로젝트 패널 하단의 휴지통 버튼은 스크롤 영역 밖 고정 푸터에 배치한다. 위의 프로젝트 목록만 스크롤하며 패널이 접히면 푸터도 숨긴다. 버튼은 기존 선형 휴지통 아이콘·보조 버튼·44px 조작 영역·세이지 배경을 사용한다. 휴지통 모달은 현재 반의 삭제한 프로젝트·Step·활동·자료와 삭제 시각, 복원 버튼을 표시하며 빈 목록·로딩·실패·재시도·복원 중 상태를 구분한다. 학생에게는 버튼·목록을 제공하지 않는다. 삭제는 영구 삭제 대신 원본 ID와 학생 기록을 보존하는 휴지통 이동이며 확인 창에서 복원 가능함을 안내한다. 현재 프로젝트를 덮어쓰지 않고, 다른 프로젝트가 있으면 먼저 휴지통으로 옮긴 뒤 원본 프로젝트를 복원하도록 오류를 안내한다. 이미 삭제된 과거 데이터의 복구는 약속하지 않는다. 기존 모달·입력·색상 토큰을 재사용한다.

- 자료 편집은 선택 입력인 `교사 설명` 일반 텍스트와 기존 서식 본문인 `복사할 내용`을 분리한다. 기존 content는 그대로 복사 본문으로 유지하며 teacherDescription이 없는 자료의 설명은 빈 값이다. 설명은 학생에게도 보이는 안내이며 패널·확대·발표에서 복사 본문 앞에 표시한다. 비어 있으면 설명 영역을 숨긴다. 복사 영역은 기존 세이지 표면·테두리·12px 모서리·16px 여백을 재사용하고 제목과 최소 44px 복사 버튼을 같은 행에 둔다. 한글은 단어 중심으로, 긴 URL·코드는 영역 안에서 줄바꿈한다. 복사에는 본문만 포함하며 줄바꿈·코드 들여쓰기·템플릿 변수 치환을 보존한다. 템플릿의 기존 생성 결과 복사 버튼은 중복하지 않는다. 이 규칙은 이전 자료 전체 복사 규칙보다 우선한다.

- 교사가 학생 개인 카드를 선택하면 현재 Step의 첫 활동을 학생 기록 조회 패널에서 자동으로 연다. 이후 활동·자료 제목 또는 `패널에서 열기`로 개별 항목을 선택한다. 패널과 확대 모달은 같은 학생 답변·저장 URL·체크 상태를 표시하고 학생 이름을 헤더에 명시한다. 학생 답변과 체크리스트는 읽기 전용이며 URL은 기존 안전한 새 탭 링크를 사용한다. 학생 조회 중 활동중 지정은 조회 대상을 바꾸지 않는다. 학생·Step·반·프로젝트가 바뀌면 이전 상세 보기를 닫고 새 범위의 기록을 표시하며, 개인 카드 목록으로 돌아가면 교사 프로젝트·시연 패널을 복구한다. 기존 패널·버튼·모달 토큰을 재사용한다. 본문 영역 너비가 420px 이하이면 개인 카드로 돌아가기 버튼과 학생 제목을 세로로 배치해 한글 단어가 글자 단위로 나뉘지 않게 한다.

- 학생 활동의 본문과 첨부 이미지 사이에 선택 입력인 `활동 URL` 목록을 둔다. 빈 목록은 입력 한 줄을 보여주고 마지막 줄 오른쪽 끝의 `+` 버튼으로 칸을 추가하고 새 칸에 포커스를 옮긴다. 추가·삭제 버튼은 44px 조작 영역과 기존 색상·입력·포커스 토큰을 사용한다. 학생은 체크리스트 및 확인 완료 여부와 관계없이 자신의 URL을 입력·수정·삭제하고 하단의 `저장` 버튼 하나로 답변과 URL을 함께 저장한다. 별도 `URL 저장` 버튼은 두지 않는다. 체크리스트가 미완료이면 내용은 저장하되 활동을 완료 처리하지 않는다. 저장한 HTTP(S) 링크는 학생 패널·확대 보기와 교사의 해당 학생 상세 보기에서 표시하며, 긴 주소는 줄바꿈하고 새 탭으로 연다. 학생별 활동 기록에만 저장하며 기존 교사 제공 링크는 유지한다. 빈 입력은 저장에서 제외하고 오류 발생 시 입력을 보존한다.

- 교사용 메인 활동·자료 카드 헤더에는 수정 아이콘 옆에 휴지통 삭제 명령을 제공한다. 기존 선형 아이콘·보조 글자색·키보드 포커스와 터치 조작 영역을 재사용하고 hover에는 `--danger` 색상을 사용한다. 학생 DOM에는 삭제 버튼을 렌더링하지 않으며 삭제 콜백도 전달하지 않는다. 삭제는 항목 이름과 범위를 알리는 확인 창을 거치고, 처리 중 중복 실행을 막으며 실패 시 오류와 재시도를 제공한다.

- 교사·학생 모두 반에 진입하거나 반을 바꾸면 메인 화면의 첫 Step을 선택한다. 접속 후 사용자가 선택한 Step과 학생의 개인 카드 보기는 같은 반의 데이터 갱신으로 바뀌지 않는다. 이전의 기본 Step 미선택 규칙보다 우선한다.
- 선택된 Step 버튼과 활동·자료 카드 사이에는 `book-step-guidance` 안내 공간을 둔다. `--card-bg`, `--border`, `--text`, 8px 모서리, 16px 안쪽 여백, 14px/1.7 본문, 최소 64px 높이를 사용한다. 일반 텍스트의 줄바꿈과 공백을 보존하고 긴 단어는 영역 안에서 줄바꿈한다. 내용이 없으면 대체 문구 없이 빈 공간을 유지한다. 프로젝트 편집의 각 Step 제목 아래에는 기본 높이 380px(16행), 최소 높이 96px이며 세로 크기를 직접 조절할 수 있는 `Step 안내` 입력란과 선택 입력 설명을 제공하며 기존 입력·포커스 색상을 사용한다.

- 공통 수업 자료는 반·차시별 체크박스로 선택하고 `배포 목록 저장`으로 반영한다. 파일 행에 현재 반 배포 상태와 전체 사용처 수를 표시하며 사용 중인 원본 삭제는 비활성화한다. 학생의 `자료 내려받기` 버튼은 배포 목록이 있을 때만 표시한다. 학생 다운로드 모달은 파일별 내려받기와 선택 내려받기를 제공하며 검색·선택·작업 상태는 모달 안에서 유지한다. 반 전환 시 이전 반의 목록과 선택을 노출하지 않는다.

- 수업 준비는 교사 소유 원본 배포 파일 목록이다. 슬라이드 변환·수업 시작·활동 편집을 제공하지 않는다. 업로드, 파일명 검색, 다운로드, 배포 링크 복사, 삭제를 제공하며 기존 슬라이드 자료는 별도 접힌 목록에서 장별 다운로드한다. 파일 행은 기존 모달·목록 토큰, 8px 간격과 44px 명령 영역을 사용하고 모바일에서 파일명과 명령을 두 줄로 배치한다. HTML과 SVG를 포함한 원본은 다운로드 전용이며 실행하지 않는다.

- 사용자 관리의 PIN 설정·재설정과 탈퇴 처리 버튼은 동일 너비의 2열과 최소 44px 높이를 사용하며 기존 기본색과 위험 동작 색상은 유지한다.

- 운영 중인 반 관리 행은 반 이름과 수정·보관·삭제 버튼을 첫째 줄 양끝에 배치한다. 가입 상태·참여 코드·코드 저장·가입 차단·자동 생성·복사·전체보기는 둘째 줄에 줄바꿈 없이 배치하며 좁은 화면에서는 이 줄만 가로 스크롤한다.

- 템플릿 활동은 일반 답변란과 답변 받기 설정을 표시하지 않고 확인 동작만 제공한다. 기존 답변 데이터는 보존하며 템플릿 확인 시 답변을 덮어쓰지 않는다. 자료를 활동으로 변환할 때 답변 받기를 자동으로 켜지 않는다.

- 자료에도 활동과 같은 템플릿 사용 설정과 변수 편집을 제공한다. 학생은 자료 패널과 확대 창에서 변수 값을 입력하고 완성된 문구를 복사한다. 두 보기의 입력값을 공유하며 템플릿 자료의 상세 화면에서는 원문 복사 버튼을 중복 표시하지 않는다.

- 활동·자료 뒤의 추가하기 버튼은 같은 목록의 카드와 가로·세로 크기를 맞춘다. 카드 그리드는 모든 행의 높이를 동일하게 유지하고, 마지막 행에 버튼만 놓여도 앞 행의 카드 높이를 따른다. 기존 연한 점선 테두리와 보조 글자색을 유지한다.

- 학교명·이름 로그인은 기존 상단 입력을 유지하고, 서버 응답에 따라 PIN 등록/입력 모달로 이어진다. PIN은 하나의 비밀번호 입력 칸과 숫자 키보드를 사용한다. 등록·교사 재설정에는 확인 입력을 추가한다. 기존 modal-auth/버튼/오류 색상 토큰을 재사용하며 작은 화면에서도 입력과 버튼이 모달 폭 안에 들어간다. 브라우저 저장 정보만으로 인증을 복원하지 않는다.

- 활동과 자료의 예전 `locked` 값은 학생·교사의 내용 열람, 복사, 확인, 답변 입력을 제한하지 않는다. 활동중 상태는 패널 자동 표시와 카드 강조만 결정한다. 반 소속 및 보관된 반의 접근 제한은 별도로 유지한다.

- 사용자 관리 목록은 기존 검색 입력과 같은 40px 반 선택 메뉴를 사용하고, 반 이름과 인원 수가 있는 구분선 헤더 아래 사용자 행을 묶는다. 보관된 반은 표시를 붙이고 소속 없는 사용자는 별도 그룹으로 둔다. 기존 색상 및 12/13px 글자 크기를 유지한다.

- 교사용 왼쪽 프로젝트 사이드바는 Step과 활동·자료 순서 정리 전용 표면이다. 상단의 `프로젝트 크게 편집`만 전체 편집 모달을 열고, 사이드바 안에는 프로젝트 제목 입력, Step 추가, Step 편집, 활동·자료 내용 편집·삭제·잠금·내보내기 명령을 두지 않는다. Step 행은 Step 순서 이동을 제공하고, 펼친 활동·자료 행은 저장된 순서대로 표시하며 행별 위/아래 이동만 제공한다. 저장 실패는 사이드바 안에 사용자에게 보이는 오류로 표시한다. 기존 메인 카드의 항목 편집·잠금 아이콘과 교사 푸터의 발표 모드는 유지하며, 이미 잠긴 활동·자료 상태는 보존한다.

- 활동·자료 메인 카드의 학생 하단 명령은 동일 너비의 `확인`과 `패널에서 열기` 두 버튼이다. 확인은 기존 체크리스트 검증과 학생별 저장을 사용하고, 저장 중·잠김·확인 완료 상태를 구분한다. 학생 패널·확대 모달의 확인/저장 기능은 유지한다. 학생 자료 헤더에도 기존 복사 아이콘을 제공한다. 교사 메인 카드 하단에는 `발표 모드`만 남기며 교사 일괄 확인은 제공하지 않는다. 이 규칙은 아래의 이전 단일 열기·잠금 버튼 설명보다 우선한다.
...
- 실제 선택된 차시의 purpose가 internal인 저장 프로젝트에 제공한다. STEP 개수·제목은 고정하지 않으며 현재 선택된 STEP과 관계없이 마지막 STEP 버튼 바로 오른쪽에 `포트폴리오 만들기` 버튼을 둔다. 기존 STEP 버튼과 같은 높이·글꼴·알약 형태를 공유하고, 좁은 화면에서도 같은 줄의 가로 스크롤 안에서 마지막 항목으로 유지한다. 교사는 학생을 선택한 문맥에서 같은 학생 기록을 열고, 미선택 시 버튼을 비활성화하고 툴팁으로 학생 선택을 안내한다. 학생은 본인의 기록만 연다.
- 생성 범위는 현재 차시·프로젝트의 저장된 모든 활동 답변, 학생 URL, 학생 캡처(마지막 STEP 포함)다. 교사의 안내·이미지, 다른 학생이나 다른 차시, 저장하지 않은 템플릿 입력은 합치지 않는다. STEP·활동 순서와 제목을 그대로 보존해 임의로 바꾼 프로젝트도 정확히 읽히게 한다. 내용을 AI로 지어내거나 임의 분류하지 않는다.
- 보고서는 기존 forest #2f6f3e, deep #24552f, ink #1f2e25, body #344238, muted #617468, paper #fbfaf3, sage #e6f1ea, border #d5e1d2, white #ffffff를 자체 CSS 변수로 내장한다. 별도 폰트 다운로드 없이 Malgun Gothic/Apple SD Gothic Neo/system sans 본문, Batang/Georgia/serif 제목, Consolas/monospace 프롬프트를 사용한다. 표지는 짧은 포트폴리오 라벨, 프로젝트 제목, 학생 이름·학교·차시·생성일, 원문 수집 설명 순서로 배치한다. 4px 단위 간격(8/12/16/24/32/48), 본문 16px/1.8, 보조 12/14px, 제목 28/40px, 섹션 24px, 8px 모서리, 최대 문서 폭 900px를 사용한다.
- 페이지 구조는 대시보드 카드 묶음 대신 여백·선으로 구분한 편집 보고서다. 실제 STEP 제목의 목차, 번호·제목의 섹션 헤더, 활동 제목·저장 시각·답변, 안전한 링크의 실제 주소, 출처 활동 바로 아래 종횡비를 유지한 캡처 순서다. 학생 원문은 텍스트로 이스케이프하고 줄바꿈·들여쓰기를 보존하며 긴 URL·프롬프트는 폭 안에서 줄바꿈한다. 프롬프트 제목을 가진 활동만 글꼴을 구분하고 의미를 재분류하지 않는다. 빈 활동은 `저장된 내용이 아직 없어요.`로 표시한다.
- HTML은 CSS·이미지를 포함한 독립 정적 문서이며 스크립트·폼·외부 폰트가 없다. 미리보기는 스크립트 권한 없는 iframe으로 격리한다. PDF는 같은 문서의 브라우저 인쇄/PDF 저장을 사용하며 `인쇄 창에서 PDF로 저장을 선택하세요.`를 버튼 근처에 표시한다. @page A4 14mm 여백, 11pt 본문, 제목/이미지 분리 방지, 긴 본문 페이지 분할 허용, URL 텍스트 유지, 배경 인쇄 꺼짐에도 읽을 수 있는 대비를 적용한다.
- 미리보기 모달은 최대 1080px, 92dvh, 기존 모달 표면·버튼 토큰을 사용한다. 헤더·저장 명령은 고정, 문서만 스크롤한다. 375/768/1280에서 단일 열 문서, 좁은 화면은 헤더 명령 두 줄과 최소 44px 조작 영역을 유지한다. Escape/배경 닫기, 포커스 가두기·복귀, 의미 있는 제목 계층을 제공한다.
- 불러오기·이미지 준비·빈 기록·실패·재시도를 구분하고 준비가 끝나기 전 다운로드를 비활성화한다. 읽기 실패를 빈 답변으로 내보내지 않는다. 누락 이미지는 출처와 누락 상태를 문서와 안내에 표시한다. 반·프로젝트·학생·역할 변경 시 모달을 닫고 진행 중 결과를 폐기한다. 미저장 내용은 포함하지 않는다는 안내와 다시 불러오기 명령을 제공한다.
- 학생 캡처는 기존 BookItemImageEditor의 파일 선택·붙여넣기·드롭·순서·삭제를 재사용하되 방송 크기는 숨긴다. 본문과 URL과 함께 같은 하단 저장 버튼으로 저장한다. 처리/저장 중 중복 저장을 막고 실패 시 초안을 유지한다. 학생 기록당 최대 8장·600000문자 제한과 전체 문서 크기 검증을 적용한다. 교사의 학생 조회에는 저장된 캡처만 읽기 전용으로 표시한다. 이미지 준비가 페이지 탐색으로 취소되면 이전 학생/활동에 반영하지 않는다.

### 학생별 차시 보고서 명시
- 교내용의 각 차시 하나당 학생 개인 보고서 하나를 생성한다. 버튼 문구는 포트폴리오 만들기를 유지하고 모달 제목은 학생별 차시 보고서, 문서 표지는 교내용 · 학생별 차시 보고서 → 차시 이름 + 보고서 → 프로젝트 제목 → 학생·학교·생성일 순서로 표시한다. 기존 표지 제목/본문 토큰을 사용한다.
- 파일명은 차시보고서-차시이름-학생이름-프로젝트이름.html로 차시를 구분한다. 차시 안의 모든 STEP을 포함하고 다른 차시를 합치지 않는다. 선택된 차시와 프로젝트 소속이 다르면 생성을 비활성화하고 불러오기도 거부한다. 차시 변경은 열린 보고서를 닫는다.
- 프롬프트의 한글은 Malgun Gothic/Apple SD Gothic Neo 대체 글꼴과 keep-all 줄바꿈을 사용해 음절 중간 분리를 피한다. 긴 공백 없는 코드는 기존 overflow-wrap:anywhere로 폭 안에 유지한다.

### 전체 화면 보고서와 자동 저장
- 보고서 오버레이는 브라우저 뷰포트의 사방을 채우고 100dvh 높이·너비100%를 사용한다. 바깥 여백과 모달 둥근 모서리를 없애며 기존 머리글·닫기·HTML/PDF 명령과 본문 스크롤을 유지한다. 좁은 화면에서는 명령 줄바꿈을 허용한다.
- 학생 패널과 확대 모달의 답변·URL·캡처·템플릿 값은 같은 초안을 공유한다. 입력 종료700ms 후 자동 저장하며 기록 저장으로 확인 완료를 바꾸지 않는다. 자동 저장 중에도 입력이 가능하고 이전 저장 응답이 최신 입력을 덮지 않는다.
- 하단에 기존 보조색/오류색과12px 본문으로 자동 저장 대기·저장 중·저장됨·실패 상태를 제공한다. 실패한 초안은 유지하고 다시 저장 버튼을 제공한다. 수동 저장/확인 명령은 기존 활동 완료 의미를 유지하며 보고서 열기 전 미저장 초안을 저장한다. 학생·차시·항목별 임시 초안을 기기에 보관해 새로고침 시 복원하며 교사 조회는 읽기 전용이다.

### 교사 포트폴리오 발표
- 준비된 학생별 차시 보고서의 명령 줄 마지막에 교사 전용 `발표 모드`를 추가한다. 같은 차시의 학생 화면에 현재 보고서의 읽기 전용 복사본을 공개한다. 발표 중에는 `발표 종료`와 학생 이름·현재 차시 공개 안내를 표시하며, 처리 중 중복 명령은 막고 실패 시 보고서를 유지한 채 재시도를 제공한다. 모달을 닫거나 다시 불러올 때 발표를 먼저 종료한다.
- 학생 발표 표면은 기존 전체 화면 보고서·forest 버튼·세이지 표면·12px 안내·44px 조작 영역을 재사용한다. `선생님이 포트폴리오를 보여주고 있어요`와 학생 이름·차시를 표시하고 문서만 스크롤한다. 학생에게 발표/종료/다운로드 명령은 제공하지 않는다. 교사의 스크롤을 따라가며 본문 URL은 새 탭에서 연다. 기존 포트폴리오의 한글 서체·줄바꿈·캡처 비율과 스크립트 없는 문서를 유지한다.
- 375/768/1280px에서 머리글이 자연스럽게 줄바꿈하고 가로 넘침 없이 보고서가 남은 높이를 채운다. 로딩·실패·재시도 상태를 별도로 표시하고 발표 종료·다른 차시 전환 시 기존 학생 화면으로 복귀한다.
