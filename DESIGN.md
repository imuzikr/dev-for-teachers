...
- `book-progress-drawer`: 교사 화면 오른쪽에 붙는 서랍형 진척도 패널이다. Step별로 활동과 자료를 사각형 상태 카드로 나누어 표시하고, 각 항목은 확인한 학생 수에 따라 색상이 단계적으로 변한다. 접힌 상태에서는 세로 레일로 남아 본문 폭을 확보한다.
...
- 교사용 오른쪽 진척도 서랍은 접기 상태를 브라우저에 기억한다. 확인 없음, 일부 확인, 절반 이상 확인, 전체 확인은 배경색과 테두리색의 단계 차이로 구분한다.
