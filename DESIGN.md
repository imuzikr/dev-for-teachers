...
- `book-project-step`: 활동과 자료를 구획으로 분리해 각각 복수로 담는 아코디언 카드. 편집 중인 Step 제목은 아코디언 헤더에서 바로 수정한다. 본문 하단에는 활동 추가와 자료 추가 명령을 나란히 두고, 항목이 생긴 경우에만 해당 편집 박스를 버튼 위에 표시한다. 자료는 텍스트 내용을 필수로 받고 선택 URL이 있으면 외부 링크로 표시한다.
