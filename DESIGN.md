...
- `book-personal-card`: 학교명, 이름, 활동 진행률을 표시한다. 교사는 모든 참여자 카드를 열어 개별 진행 상황을 확인할 수 있고, 일반 사용자는 자신의 카드만 열 수 있다. 교사용 개인 카드 섹션 헤더에는 프로젝트 항목별 학급 평균 확인 상태바를 표시한다. 각 칸은 활동·자료 하나를 뜻하고 Step 경계는 간격으로 구분하며, 칸 내부 채움 비율은 해당 항목을 확인한 학생 수를 전체 참여자 수로 나눈 값으로 표현한다. 평균 상태바 색은 연한 주황색을 사용한다.
