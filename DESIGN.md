...
- 교사용 메인 활동·자료 목록은 남은 가운데 폭을 나눠 쓰는 반응형 격자로 배치한다. 카드의 최소 읽기 폭 220px를 확보할 수 없으면 다음 줄로 배치하며, 오른쪽 패널을 펼칠 때 카드나 하단 버튼이 가로 스크롤 영역 밖으로 잘리지 않아야 한다. 학생의 기존 가로 탐색 방식은 유지한다.
