...
- 선택된 Step 버튼과 활동·자료 카드 사이에는 `book-step-guidance` 안내 공간을 둔다. `--card-bg`, `--border`, `--text`, 8px 모서리, 16px 안쪽 여백, 14px/1.7 본문, 최소 64px 높이를 사용한다. 일반 텍스트의 줄바꿈과 공백을 보존하고 긴 단어는 영역 안에서 줄바꿈한다. 내용이 없으면 대체 문구 없이 빈 공간을 유지한다. 프로젝트 편집의 각 Step 제목 아래에는 4행 `Step 안내` 입력란과 선택 입력 설명을 제공하며 기존 입력·포커스 색상을 사용한다.
